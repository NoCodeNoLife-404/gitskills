...
static void UT0_set_baud(u32 baud)
{
    ...
    if (JL_UART0->CON0 & BIT(5)) {
        if (uart0.rx_timeout > 10) {
            JL_UART0->OTCNT = (uart0.rx_timeout / 10) * (UART_CLK / 10) / 10;
        } else {
            JL_UART0->OTCNT = uart0.rx_timeout * UART_CLK / 1000;
        }
    }
...
static void UT1_set_baud(u32 baud)
{
    ...
    if (JL_UART1->CON0 & BIT(5)) {
        if (uart1.rx_timeout > 10) {
            JL_UART1->OTCNT = (uart1.rx_timeout / 10) * (UART_CLK / 10) / 10;
        } else {
            JL_UART1->OTCNT = uart1.rx_timeout * UART_CLK / 1000;
        }
    }
...
static void UT2_set_baud(u32 baud)
{
    ...
    if (JL_UART2->CON0 & BIT(5)) {
        if (uart2.rx_timeout > 10) {
            JL_UART2->OTCNT = (uart2.rx_timeout / 10) * (UART_CLK / 10) / 10;
        } else {
            JL_UART2->OTCNT = uart2.rx_timeout * UART_CLK / 1000;
        }
    }
...
static u8 ut_tx_pin[] = {
    IO_PORTA_05, IO_PORTB_02, IO_PORTA_01, IO_PORTB_08,
    IO_PORTB_04, IO_PORTB_00, IO_PORTA_07,  IO_PORT_DP,
    IO_PORTA_03, IO_PORTA_00, IO_PORTB_06, IO_PORT_DP1
};
static u8 ut_rx_pin[] = {
    IO_PORTA_06, IO_PORTB_03, IO_PORTA_02, IO_PORTB_09,
    IO_PORTB_05, IO_PORTB_01, IO_PORTA_08,  IO_PORT_DM,
    IO_PORTA_04, IO_PORTA_00, IO_PORTB_07, IO_PORT_DM1
};
