...
#if (CONFIG_MESH_MODEL == SIG_MESH_VENDOR_CONTROL2)
 ...
 */
/*-----------------------------------------------------------*/
const u8 config_bt_mesh_adv_buf_count = 20;

/**
 * @brief Conifg complete local name
 */
/*-----------------------------------------------------------*/
#define BLE_DEV_NAME        'V', 'd', '_','c', 't', 'r', '2'
...
BT_MESH_MODEL_PUB_DEFINE(vendor_pub_cli1, NULL, MAX_USEFUL_ACCESS_PAYLOAD_SIZE);
BT_MESH_MODEL_PUB_DEFINE(vendor_pub_cli2, NULL, MAX_USEFUL_ACCESS_PAYLOAD_SIZE);
...
static struct bt_mesh_model vendor_control_models[] = {
    BT_MESH_MODEL_VND(BT_COMP_ID_LF, BT_MESH_VENDOR_MODEL_ID_CLI,
    vendor_cli_op, &vendor_pub_cli, &onoff_state[0]),
};

static struct bt_mesh_model vendor_control_models_1[] = {
    BT_MESH_MODEL_VND(BT_COMP_ID_LF, BT_MESH_VENDOR_MODEL_ID_CLI,
    vendor_cli_op, &vendor_pub_cli1, &onoff_state[1]),
};

static struct bt_mesh_model vendor_control_models_2[] = {
    BT_MESH_MODEL_VND(BT_COMP_ID_LF, BT_MESH_VENDOR_MODEL_ID_CLI,
    vendor_cli_op, &vendor_pub_cli2, &onoff_state[2]),
};
...
static struct bt_mesh_model *mod_cli_sw[] = {
    &vendor_control_models[0],
    &vendor_control_models_1[0],
    &vendor_control_models_2[0],
};
...
void input_key_handler(u8 key_status, u8 key_number)
{
    ...
    if ((key_number == 4) && (key_status == KEY_EVENT_LONG)) {
        log_info("\n  <bt_mesh_reset> \n");
        bt_mesh_reset();
        return;
    }
