...
--[[=============================== 配置子项1-0: charge_en ================================--]]
local en = cfg:enum("充电配置使能开关:" .. TAB_TABLE[1], ENABLE_SWITCH, 0)
en:setOSize(1)
en:setTextOut(
	function (ty) 
		if (ty == 3) then
...
--[[=============================== 配置子项1-1: poweron_en ================================--]]
local poweron_en = cfg:enum("开机充电使能开关:" .. TAB_TABLE[1], ENABLE_SWITCH, 0)
...
local off_poweron_en = cfg:enum("充电拔出自动开机使能:" .. TAB_TABLE[1], ENABLE_SWITCH, 0)
...
--[[=============================== 配置子项1-3: full_v ================================--]]
local full_v = cfg:enum("充电满电压:" .. TAB_TABLE[1], CHARGE_FULL_VOL, 8)
full_v:setOSize(1)
full_v:setTextOut(
	function (ty) 
		if (ty == 3) then
...
--[[=============================== 配置子项1-4: full_mA ================================--]]
local full_mA = cfg:enum("充电满电流:" .. TAB_TABLE[1], CHARGE_FULL_MA, 3)
full_mA:setOSize(1)
full_mA:setTextOut(
	function (ty) 
		if (ty == 3) then
...
--[[=============================== 配置子项1-5: mA ================================--]]
local mA = cfg:enum("充电电流:" .. TAB_TABLE[1], CHARGE_MA, 3)
mA:setOSize(1)
mA:setTextOut(
	function (ty) 
		if (ty == 3) then
...
local charge_default_button_view = cfg:stButton(" CHARGE 充电配置恢复默认值 ", reset_to_default(charge_cfg_default_table));

local charge_view_list = cfg:vBox {
    cfg:stHScroll(cfg:vBox {charge_group_view})
};
