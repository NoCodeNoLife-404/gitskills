...
local audio_volume_en = cfg:enum("音量配置使能开关：", ENABLE_SWITCH, 0)
...
cfg:addGlobalConstraint(
	function () 
		return default_vol.val <= max_vol.val  or "系统默认音量不能大于系统最大音量";
	end
);
...
local audio_volume_group_view = cfg:stGroup("音量配置",
...
local audio_view = {"AUDIO 配置", audio_view_list};
